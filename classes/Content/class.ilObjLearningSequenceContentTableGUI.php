...
class ilObjLearningSequenceContentTableGUI extends ilTable2GUI
{
    public function __construct(
        ...
        $this->addColumn($this->lng->txt("Sichtbarkeit"));
        $this->addColumn($this->lng->txt("Schwelle"));
        $this->addColumn($this->lng->txt("abhängige Objekte"));
    ...
    protected function fillRow($set)
    {
        ...
        $test3 = new ilCheckboxInputGUI(
            "",
            $this->parent_gui->getFieldName($this->parent_gui::FIELD_TEST3, $set->getRefId())
        );
        $test3->setChecked($set->getVisibility()->getVis());
        $this->tpl->setVariable("TEST3", $test3->render());

        $test = new ilSelectInputGUI(
            "",
            $this->parent_gui->getFieldName($this->parent_gui::FIELD_TEST, $set->getRefId())
        );
        $testoptions = $this->parent_gui->getPossiblePostConditionsForType($set->getType());
        $testArray = array(
            'nothing' => 'nichts',
            'above' => 'darüber',
            'below' => 'darunter'
        );
        $test->setOptions($testArray);
        $test->setValue($set->getVisibility()->getVisibilityOperator());


        $test2 = new ilTextInputGUI(
            "",
            $this->parent_gui->getFieldName($this->parent_gui::FIELD_TEST2, $set->getRefId())
        );
        $test2->setSize(2);
        $test2->setDisabled($this->set_is_used);
        $test2->setValue($set->getVisibility()->getPre());
        ...
        $this->tpl->setVariable("TEST", $test->render());
        $this->tpl->setVariable("TEST2", $test2->render());
