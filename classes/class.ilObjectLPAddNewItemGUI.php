...
class ilObjectLPAddNewItemGUI
{
    ...
    protected function parseRepository()
    {
        ...
        $subtypesLso = $objDefinition->getCreatableSubObjects("lso", $this->mode, $this->parent_ref_id);

        $parent_type = "cat";
        $allSubtypes = $objDefinition->getSubObjects($parent_type);

        $subtypesCat=array(
            'cmix' => $allSubtypes['cmix'],
            'xhfp' => $allSubtypes['xhfp'],
        );
        $subtypes = array_merge($subtypesCat, $subtypesLso);
        if (count($subtypes) > 0) {
