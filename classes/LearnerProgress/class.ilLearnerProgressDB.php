...
class ilLearnerProgressDB
{
    ...
    public function getLearnerItems(int $usr_id, int $container_ref_id) : array
    {
        ...
        $resultIds = $test->getLso();
        if($resultIds == null){
            return $items;
        }
        $realLso = [];
        foreach ($items as $realitems){
            if(in_array($realitems->getRefId(), $resultIds)){
                $realLso[] = $realitems;
            }
        }
        return $realLso;
    ...
    {
        file_put_contents('console.log', print_r(var_export($ls_item, true) . "\n", true), FILE_APPEND);
        ...
        file_put_contents('console.log', print_r(var_export($access, true) . "\n", true), FILE_APPEND);
