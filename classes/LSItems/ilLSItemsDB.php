 ...
 */
include_once("/var/www/html/ilias/Modules/LearningSequence/classes/LSItems/UserLSItemDB.php");
class ilLSItemsDB
{
    /**
     * @var ilTree
    ...
    public function getLSItems(int $ref_id) : array
    {
        ...
        $visibility = $this->getVisibilityForChildren($children);
        $items = [];
    ...
    protected function getVisibilityForChildren(array $children) : array
    {
        $ref_ids = array_map(
            function ($i) {
                return (int) $i['child'];
        ...
        $conditions = [];
        foreach ($this->visibilityDB->select($ref_ids) as $condition) {
            $conditions[$condition->getRefId()] = $condition;
        }
    ...
    protected function storeVisibility(array $ls_items)
    {
        $conditions = [];
        foreach ($ls_items as $item) {
            $conditions[] = $item->getVisibility();
        }
        $this->visibilityDB->upsert($conditions);
    }
