...
class ilLearningSequenceImporter extends ilXmlImporter
{
    ...
    protected function buildLSItems(array $ls_data, ilImportMapping $mapping)
    {
        ...
        foreach ($ls_data as $data) {
            ...
            file_put_contents('console.log', print_r(var_export("der type hier", true) . "\n", true), FILE_APPEND);
            file_put_contents('console.log', print_r(var_export($data, true) . "\n", true), FILE_APPEND);

            //$visibiliy = new ilLSVisibility(
            //    (int) $new_ref_id
            //);
